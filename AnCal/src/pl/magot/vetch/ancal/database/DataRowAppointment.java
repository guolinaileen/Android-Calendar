...
public class DataRowAppointment extends DataRow
{
	...
	private final DataField[] TableDef = {
			...
			new DataField(fid.Address, "Address", DataField.Type.TEXT, true, false),
			new DataField(fid.Contacts, "Contacts", DataField.Type.TEXT, true, false)
	};
	...
	{
		sAddress = new String(value.trim());
	}
...
*/
	...
	public void SetValuesForDataRow()
	{
		...
		Value(fid.Address).set(GetAddress());
		Value(fid.Contacts).set(GetContacts());
	...
	public void GetValuesFromDataRow()
	{
		...
		SetAddress(Value(fid.Address).asString());
		...
		SetContacts(Value(fid.Contacts).asString());
